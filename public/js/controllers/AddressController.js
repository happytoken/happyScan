    ...
    var fetchTxs = function() {
      var table = $("#table_txs").DataTable({
        ...
        "columnDefs": [
          ...
          { "render": function(data, type, row) {
						var tmp = data;
						if (JSON.stringify(row).indexOf(',"0x7ff36ab5')>0) {
							  tmp = 'Sell';
						  } else if (JSON.stringify(row).indexOf(',"0x18cbafe5')>0) {
							  tmp = 'Buy';
						  } else if (JSON.stringify(row).indexOf(',"0xf305d719')>0) {
							  tmp = 'Add';
						  } else if (JSON.stringify(row).indexOf(',"0xded9382a')>0) {
							  tmp = 'Remove';
						  } else {
							  tmp = 'Tansfer';
						  }
                        return '<a href="/block/'+data+'">'+ tmp +'</a>' // zyl exchange
                      }, "targets": [1]},
          { "render": function(data, type, row) {
