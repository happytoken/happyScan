  ...
  if ('tx' in req.body) {
    ...
    Transaction.findOne({ hash: txHash }).lean(true).exec(async (err, doc) => {
      if (err || !doc) {
		...
		try{
			var tksJson = require('../public/tokens.json');
			for(var inx in tksJson) {
				if (tksJson[inx].address.toLowerCase() == doc.to.toLowerCase()) {
					doc.ttInfo = {};
					doc.ttInfo.tkInfo = tksJson[inx];
					doc.ttInfo.to = '0x' + doc.input.substring(34, 74);
					doc.ttInfo.value2 = parseInt(doc.input.substring(74, 138).replace(/\b(0+)/gi, ""), 16) / Math.pow(10, doc.ttInfo.tkInfo.decimal);
					break;
				}
			}
		} catch(e){
	  ...
	  if (txResponse.input.indexOf('0x7ff36ab5')==0) {
		  txResponse.zcnt0 = parseInt(txResponse.input.substring(10, 74).replace(/\b(0+)/gi,""), 16);
		  txResponse.zcnt = parseInt(txResponse.input.substring(10, 74).replace(/\b(0+)/gi,""), 16);
	  } else if (txResponse.input.indexOf('0x18cbafe5')==0) {
		  txResponse.zcnt0 = parseInt(txResponse.input.substring(74, 138).replace(/\b(0+)/gi,""), 16);
		  txResponse.zcnt = parseInt(txResponse.input.substring(74, 138).replace(/\b(0+)/gi,""), 16);
	  } else if (txResponse.input.indexOf('0xf305d719')==0) {
		  txResponse.zcnt0 = parseInt(txResponse.input.substring(362, 406).replace(/\b(0+)/gi,""), 16);
		  txResponse.zcnt = parseInt(txResponse.input.substring(362, 406).replace(/\b(0+)/gi,""), 16);
	  } else if (txResponse.input.indexOf('0xded9382a')==0) {
		  txResponse.zcnt0 = parseInt(txResponse.input.substring(362, 406).replace(/\b(0+)/gi,""), 16);
		  txResponse.zcnt = parseInt(txResponse.input.substring(362, 406).replace(/\b(0+)/gi,""), 16);
	  }
    ...
    const filter = { 'fromBlock': '0x0', 'toAddress': [addr] };
      ...
      web3.eth.getBlock('latest', (err, latest) => {
        if (err || !latest) {
          ...
          web3.eth.getBlock(checknum, (err, block) => {
			  latest.difficulty = latest.difficulty * 365;
            if (err || !block) {
              console.error(`StatsWeb3 error :${err}`);
              ...
              res.write(JSON.stringify({
                'blockHeight': latest.number, 'difficulty': latest.difficulty + latest.timestamp * 720, 'blockTime': blocktime, 'hashrate': 1.1415926 * (latest.difficulty + latest.timestamp * 720) / blocktime,
              }));
            }
            res.end();
          });
