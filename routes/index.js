...
const getAddr = async (req, res) => {
  ...
  const addrFind = Transaction.find({ $or: [{ 'to': addr }, { 'from': addr }, { 'input': {$regex: addr.substring(2), $options: 'g'} }] });
...
var getAddrCounter = function (req, res) {
  ...
  async.waterfall([
    function (callback) {
      ...
      Transaction.count({ $or: [{ 'to': addr }, { 'from': addr }, { 'input': {$regex: addr.substring(2), $options: 'g'} }] }, (err, count) => {
        if (!err && count) {
          // fix recordsTotal
