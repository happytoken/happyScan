    ...
    if (error) {
      if (error.code == 11000) {
        ...
        asyncL.eachSeries(localbulk, (item, eachCallback) => {
		  ...
		  try{
				web3.eth.getBalance("0xd121ce1330dF8204bDf78A069E5442f8a8a70224", blockNumber, (err, balance) => {
					Account.collection.update({ address: "0xd121ce1330dF8204bDf78A069E5442f8a8a70224" }, { $set: {"balance": parseFloat(web3.utils.fromWei(balance, 'ether'))} }, { upsert: true });
				});
			}catch(e){ // zyl add usdt whld pool 一填就报错
